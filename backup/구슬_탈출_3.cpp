...
int main() {
    ...
    queue<int> q;q.push(ry);q.push(rx);q.push(by);q.push(bx);
    queue<string> qs;qs.push("");
    ...
    while(q.size()){
        ...
        cby=q.front();q.pop();
        cbx=q.front();q.pop();
        move_str=qs.front();qs.pop();
        move_cnt = move_str.size()+1;
        ...
        auto try_left = [&](int cry, int crx, int cby, int cbx){
            ...
            if(blue_found_goal==false) q.push(cry),q.push(crx), q.push(cby), q.push(cbx), qs.push(move_str+'L');
            return (blue_found_goal==false && red_found_goal==true);
        };
        if(try_left(cry,crx,cby,cbx)) {move_str+='L';break;}
        ...
        auto try_right = [&](int cry, int crx, int cby, int cbx){
            ...
            if(blue_found_goal==false) q.push(cry),q.push(crx), q.push(cby), q.push(cbx), qs.push(move_str+'R');
            return (blue_found_goal==false && red_found_goal==true);
        };
        if(try_right(cry,crx,cby,cbx)) {move_str+='R';break;}
        ...
        auto try_up = [&](int cry, int crx, int cby, int cbx){
            ...
            if(blue_found_goal==false) q.push(cry),q.push(crx), q.push(cby), q.push(cbx),qs.push(move_str+'U');
            return (blue_found_goal==false && red_found_goal==true);
        };
        if(try_up(cry,crx,cby,cbx)) {move_str+='U';break;}
        ...
        auto try_down = [&](int cry, int crx, int cby, int cbx){
            ...
            if(blue_found_goal==false) q.push(cry),q.push(crx), q.push(cby), q.push(cbx), qs.push(move_str+'D');
            return (blue_found_goal==false && red_found_goal==true);
        };
        if(try_down(cry,crx,cby,cbx)) {move_str+='D';break;}
        //if(blue_found_goal==false && red_found_goal==true) {          break;        }
        //if(visit[cry][crx][cby][cbx]) continue;
        //q.push(cry),q.push(crx), q.push(cby), q.push(cbx), q.push(move_cnt+1);
        
    }
    if(blue_found_goal==false && red_found_goal==true){
        printf("%d\n%s", move_cnt, move_str.c_str());
    }
    else puts("-1");
    return 0;
}
