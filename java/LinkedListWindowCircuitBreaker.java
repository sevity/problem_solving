...
public class LinkedListWindowCircuitBreaker {
    private final double FAILURE_THRESHOLD = 0.5; // 실패율 임계값, 예: 50%
    private static final long SLIDING_WINDOW_TIME = 10000; // 예: 10초

    private LinkedList<Long> failureTimestamps = new LinkedList<>();  // 실패한 경우에만 기록
    private LinkedList<Long> totalRequestsTimestamps = new LinkedList<>();  // 성공해도 실패해도 기록
    ...
    public Response rpcProxy(int param1, String param2) throws APIException {
        ...
        // 오래된 실패 이벤트 제거
        while (!failureTimestamps.isEmpty() && (currentTime - failureTimestamps.getFirst() > SLIDING_WINDOW_TIME)) {
            failureTimestamps.removeFirst();
        }
        // 오래된 전체 요청 이벤트 제거(성공/실패 모두 포함)
        while (!totalRequestsTimestamps.isEmpty() && (currentTime - totalRequestsTimestamps.getFirst() > SLIDING_WINDOW_TIME)) {
            totalRequestsTimestamps.removeFirst();
        }
        ...
        try {
            ...
            totalRequestsTimestamps.addLast(currentTime); // 전체 요청 타임스탬프 추가
            return response;
        } catch (APIException e) {
            failureTimestamps.addLast(currentTime); // 실패 이벤트 타임스탬프 추가
            totalRequestsTimestamps.addLast(currentTime); // 전체 요청 타임스탬프 추가
            throw e;
        }
    }
    ...
    public static void main(String[] args) {
        LinkedListWindowCircuitBreaker circuitBreaker = new LinkedListWindowCircuitBreaker();
